...
suite "Configuration Defaults Contract":
  test "default particleCount within buffer limits":
    ## GPU shaders and typed arrays are sized for MAX_PARTICLES; exceeding it
    ## causes buffer overflows and undefined behavior.
...
suite "Configuration Invariants":
  test "friction coefficient in valid range":
    ## Friction = 0 means no damping, friction = 1 means instant stop; values
    ## outside this range cause energy injection or negative velocity.
    check DEFAULT_FRICTION >= 0.0
    check DEFAULT_FRICTION <= 1.0

  test "trailAlpha value in valid range":
    ## This is a blend factor - 0 = instant fade, 1 = permanent trails; values
    ## outside this range produce visual artifacts.
    check DEFAULT_TRAIL_ALPHA >= 0.0
    check DEFAULT_TRAIL_ALPHA <= 1.0

  test "timeScale produces stable simulation":
    ## Zero or negative timeScale stops or reverses time incorrectly; values
    ## above 2.0 destabilize the Euler integration, causing explosions.
    check DEFAULT_TIME_SCALE > 0.0
    check DEFAULT_TIME_SCALE <= 2.0

  test "forceStrength is positive":
    ## Zero force strength makes particles static; negative values invert
    ## attraction/repulsion semantics unexpectedly.
    check DEFAULT_FORCE_STRENGTH > 0.0

  test "maxVelocity is positive":
    ## This caps particle speed to prevent tunneling through boundaries; zero
    ## or negative values break velocity clamping logic.
    check DEFAULT_MAX_VELOCITY > 0.0

  test "glowIntensity is non-negative":
    ## Glow is additive - negative values would subtract light; zero disables
    ## glow (valid), positive adds bloom effect.
  ...
  test "glow knob defaults reproduce the legacy hard-coded glow radius":
    ...
    ## product must equal 12.0, preserving the appearance the glow knobs replaced,
    ## so shipping them does not change the default look.
