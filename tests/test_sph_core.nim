...
suite "XSPH Smoothing Carries No Time Factor":
  ...
  # pressure acceleration over FRAME_DT_REFERENCE plus the velocity blend.
  # integrate applies the substep's frame factor once to the summed delta.

  test "the correction is epsilon times the weight times the velocity gap":
    check xsphVelocityCorrection(3.0, 11.0, 0.4, SPH_XSPH_EPSILON) ==
      SPH_XSPH_EPSILON * 0.4 * (11.0 - 3.0)

  test "a pair with no pressure delivers the bare velocity blend":
    # Zero pressure on both sides leaves only the blend, which must reach the
    # pair delta with no clock multiplying it.
    let delta = sphPairVelocityDelta(0.0, 1.0, 0.0, 1.0, 0.5, 0.3, 2.0, 1.5,
      0.2, 0.8, (x: 0.6, y: 0.8), (x: 4.0, y: -2.0))
    let coefficient = (0.2 + SPH_XSPH_EPSILON) * 0.3 / 2.0
    check abs(delta.x - 0.8 * coefficient * 4.0) < EPSILON
    check abs(delta.y - 0.8 * coefficient * -2.0) < EPSILON
