...
`blur.wgsl`, and `colormap_core.nim` mirrors `colormap.wgsl`. Each has a test module
(`tests/test_physics.nim`, `test_grid.nim`, `test_sph_core.nim`, `test_field_core.nim`,
`test_bloom_core.nim`, `test_colormap_core.nim`), and every one of the six is exercised.

The simulation SHALL NOT call an oracle's mirrored functions — no call site for any of them exists
in `src/`. Absence of a caller is the designed state, not evidence of dead code
(`physics_core.nim:12-16`, `grid_core.nim:11-16`). Four of the six modules do appear in `src/` imports —
`sph_core`, `field_core`, `bloom_core`, `colormap_core` are imported by `config_ranges.nim`,
`shader_config.nim`, `sim_registry.nim`, `webgpu_compute.nim`, `web_api.nim`, and the
`ui/state/` modules — and those imports SHALL be for the modules' constants, never for the mirrored
math. A tool judging deadness by import count will misread `physics_core` and `grid_core`, which no
`src/` module imports at all.

#### Scenario: An oracle has no importer
- **WHEN** `src/` is searched for importers of `physics_core` or `grid_core`
- **THEN** none is found, and this satisfies the requirement rather than violating it

#### Scenario: An oracle's math is exercised
- **WHEN** `just test` runs
- **THEN** each of the six oracle modules is linked by its test module and its functions are evaluated
...
(`src/shader_config.nim:185`) emits the substitutions, and `tools/wgsl_bundle.nim` inlines them; an
unresolved `{{...}}` aborts the bundle rather than reaching the GPU (`tools/wgsl_bundle.nim:152-157`).
`PlaceholderSources` (`tools/wgsl_bundle.nim:218-229`) lists the Nim modules whose edits must trigger a
...
`just check` SHALL run the native suite and the TypeScript suite in that order (`justfile:45-50`), and
the release workflow SHALL run `just check` before `just release` (`.github/workflows/release.yml:57-61`).
A red suite in either language blocks the release.

The TypeScript suite (`just test-ui`, `bun test` over `web-ui/test/`) SHALL cover only panel-side pure
logic that has no Nim owner: value formatting (`format.test.ts`), preset storage (`presets.test.ts`), and
mode-gating arithmetic (`mode-gating.test.ts`). It SHALL NOT restate a number the Nim side serves.

`mode-gating.test.ts` carries a stated limit: its mode and group fixtures mirror the taxonomy Nim serves
rather than reading it, so the test proves the gating arithmetic and not the fixtures' agreement with
`sim_registry.controlGroupsFor`. That agreement is asserted natively on the Nim side instead
(`tests/test_sim_registry.nim:242-291`), which is the pattern this strategy prefers: assert the relation
where both tables are in reach.
...
- **WHEN** Nim changes which control groups a mode offers and the TypeScript fixture is not updated
- **THEN** `bun test` still passes, and the native registry test is what holds the real relation
