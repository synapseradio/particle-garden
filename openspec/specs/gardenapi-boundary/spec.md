...
`src/web_api.nim:838` as the last statement of the module, and `web/index.html` loads `app.js`
...
the panel with `tsc --noEmit` (`justfile:29-30`) against the declared surface in
...
(`src/web_api.nim:118-132`), which write the typed state record and then call
`applySimulationToConfig` / `applyRenderToConfig` (`src/web_api.nim:77-116`) to mirror it into the
...
`src/web_api.nim:10-18` and `web-ui/src/garden-api.ts:7-9` — and no test or compile-time assertion
checks it, because `web_api.nim` compiles only on the JS backend and no native test imports it
(`src/web_api.nim:28-31`).
...
before any store sees it (`src/web_api.nim:344-348`). No value outside a parameter's range can reach
CONFIG regardless of what the panel sends. `getParam` likewise answers only known ids and warns on
anything else (`src/web_api.nim:303-342`).
...
by `tests/test_param_descriptor.nim:290-306`; the table's own content belongs to
...
at module-eval time (`src/web_api.nim:152-172`, exposed at line 743), the simulation-mode catalog
carrying each mode's id, label, particle ceiling, and control groups (`src/web_api.nim:631-647`), the
palette-scheme catalog (`src/web_api.nim:657-664`), the colormap catalog
(`src/web_api.nim:675-682`), the preset storage keys (`src/web_api.nim:810-815`), and the species and
matrix-cell colors computed by Nim color math (`src/web_api.nim:785-794`). TypeScript SHALL restate
...
fails the Nim build if the ramp count and the index bounds disagree (`src/web_api.nim:666-673`). The
...
returns the Float32Array itself, not a copy (`src/web_api.nim:784`). The boundary SHALL provide
`isReady()` and `onReady(callback)`, queueing callbacks registered early and invoking a callback
registered late immediately (`src/web_api.nim:442-452, 737-740`). `app.nim` calls `signalReady()` at
the end of `init()` (`src/app.nim:403`), which flushes the queue and answers `isReady()` true from
then on.
...
The boundary SHALL expose `onStats(callback)` for subscription (`src/web_api.nim:805-806`) and
SHALL push samples from the frame loop rather than answering polls: `pushStats`
(`src/web_api.nim:456-474`) is called by `app.nim` inside the frame loop's cadence window
(`src/app.nim:249-250, 278-280`). The cadence is the loop's decision, not the panel's. A sample
...
allocating anything (`src/web_api.nim:461-462`).

Enforcement: the build. The sample shape is declared in `web-ui/src/garden-api.ts:50-63` and checked
...
(`src/web_api.nim:480-535, 819-822`). `applyPresetJson` SHALL return an `{ok, error}` outcome and
never throw (`src/web_api.nim:823-836`): it pre-checks parseability through the `jsonParseable` FFI
helper (`src/web_api.nim:58-63`) because `std/json.parseJson` on the JS backend delegates to
...
`schemaVersion` newer than the build understands (`src/preset.nim:425-479`). Application SHALL walk
`presetApplySteps()` in its fixed order (`src/web_api.nim:537-619`,
...
default name (`src/web_api.nim:810-815`, `src/ui/presets/preset_store_core.nim:24-27`) — so saved
...
(`src/web_api.nim:695-727`).

Enforcement: `tests/test_preset.nim` pins validation, clamping, defaulting, and round-trip
stability; `tests/test_preset_store_core.nim:55-83` pins the apply-order sequence and the storage
