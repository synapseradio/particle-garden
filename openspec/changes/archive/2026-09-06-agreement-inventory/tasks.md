...
- [x] 1.1 Write `tests/test_agreements.nim` with the six predicates as named functions over an
...
- [x] 1.2 Write `src/agreements.nim`: an `AgreementTier` enum (`Derived`, `BuildAsserted`,
...
- [x] 1.3 Seed `src/agreements.nim` with five entries, each anchor verified present:
...
- [x] 1.4 Add the suite "The Inventory Gate Can Fail" to `tests/test_agreements.nim`, running each
      ...
      restoring it.
- [x] 1.5 Register the module: add `import test_agreements` to `tests/test_all.nim` and
...
- [x] 2.1 In `tests/test_agreements.nim`, add a test asserting `src/ui/state/matrix_state.nim`
...
- [x] 2.2 In `src/ui/state/matrix_state.nim`, delete the `MATRIX_SIZE` const at line 23 and read
...
- [x] 2.3 In `src/web_api.nim`, change line 1274's `matrixStride` proc to return
...
- [x] 2.4 In `tests/test_matrix.nim`, change `MATRIX_SIZE` to `SPECIES_COUNT_MAX` at lines 23 and
      24. `config_ranges` is imported at line 2.
- [x] 2.5 Delete the test at `tests/test_memory_layout.nim:93-98` with its comment. The comment
      states "Nothing forces them to move together", which the derivation makes false.
- [x] 2.6 Rewrite the comment at `src/memory_layout.nim:185-195`, dropping the sentences about the
...
- [x] 2.7 Delete the `species-ceiling-matrix` entry from `src/agreements.nim`. Before deleting,
      run `nim c -r tests/test_agreements.nim` and watch the anchor gate fail on
      `MATRIX_SIZE* = 12`, which is the entry retiring itself.
- [x] 2.8 `just happen` and `just check` green.
- [x] 2.9 Agent verification: launch the built app, open the panel, and set the species count to its
...
- [x] 3.1 In `tests/test_agreements.nim`, add the placeholder consumption test: every key
...
- [x] 3.2 In `src/shader_config.nim`, export
      ...
      rule has one home.
- [x] 3.3 In `src/shader_config.nim`, emit `TUNABLE_BLAST_RANGE` beside `TUNABLE_BLAST_RANGE_SQ` at
...
- [x] 3.4 In `tests/test_shader_config.nim`, add a test that the emitted `TUNABLE_BLAST_RANGE`
...
- [x] 3.5 In `web/shaders/src/forces.wgsl`, declare
...
- [x] 3.6 Delete the `blast-radius` entry from `src/agreements.nim`. Before deleting, run
      `nim c -r tests/test_agreements.nim` and watch the anchor gate fail on both forces.wgsl
      anchors.
- [x] 3.7 `just happen` and `just check` green. `just happen` runs the shader stage, so a
      placeholder the map does not emit stops the build here (`tools/wgsl_bundle.nim:122-127`).
- [x] 3.8 Agent verification: launch the built app and double-click on a dense cluster. Particles
...
- [x] 4.1 In `tests/test_agreements.nim`, add a source-sweep test: `src/grid.nim` contains
...
- [x] 4.2 In `tests/test_grid.nim`, add a case pinning `computeGridDims(3840, 2160, radius)` at the
...
- [x] 4.3 In `src/grid.nim`, add `import grid_core` and replace lines 40-45 with a call to
...
- [x] 4.4 Rewrite the header at `src/grid_core.nim:9-13`, which reads "Nothing in src/ imports this
...
- [x] 4.5 Delete the `grid-dimensions` entry from `src/agreements.nim`. Before deleting, run
      `nim c -r tests/test_agreements.nim` and watch the anchor gate fail on
      `cellSize = max(CONFIG.interactionRadius, 16)`.
- [x] 4.6 `just happen` and `just check` green.
- [x] 4.7 Agent verification: launch the built app and watch the default preset for several seconds.
...
- [x] 5.1 Write `docs/agreements.md`: what a two-sided agreement is, the five tiers with the tree's
...
- [x] 5.2 In `tests/test_agreements.nim`, add a test that `docs/agreements.md` exists, is non-empty,
      and contains none of the entry keys in `AGREEMENTS`. Run it and watch it fail before 5.1
      lands, then green after.
- [x] 5.3 Rename the test named `"speciesCount clamps into [2, 8]"` in `tests/test_preset.nim` so
      ...
      do not change.
- [x] 5.4 Add a `CLAUDE.md` line under "Where authority lives" naming `src/agreements.nim` as the
      home of the agreement inventory, matching the existing one-home-per-fact entries.
- [x] 5.5 `just happen` and `just check` green.
- [x] 5.6 Read `src/agreements.nim` and confirm two entries remain, `species-ceiling-preset` and
