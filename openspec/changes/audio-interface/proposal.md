...
- **This change consumes the control matrix.** The matrix spine (row model, arbitration, the modulate arm) is introduced by the sibling change `midi-interface`. Whichever change proceeds first carries the matrix implementation with it, and this proposal stays true under either order: the work here is capture, the feature core, and family registration, with no matrix code of its own.
