...
No deployment or data migration. Implementation order within the change, under either sibling order: gate spike, then the binding and capture chain behind the affordance, then the feature core with its tests, then matrix registration, then meters and states, then shipped rows and help. If this change proceeds before midi-interface, the matrix spine (row model, arbitration, flush, modulate arm) is the prerequisite that travels forward with it, exactly as the proposal states. Rollback is removal of the affordance and the family registration, leaving any user rows naming audio sources inert and visible per interface need 5.
