...
WGSL compiler, so a bundled shader that declares a binding the pipeline does not provide, or names an
entry point that does not exist, still passes the build intact and fails on the device.

**One narrow class is now caught at build time, and the bundler SHALL fail on it.** A struct
constructor written with named fields — `Camera(centerX: x, zoom: z)` — is a WGSL parse error, since
WGSL constructors are positional only. `src/wgsl_lint.nim` is a pure function over shader source that
detects it, `tools/wgsl_bundle.nim` calls it, and the build quits naming the shader and the line.

This class earns build-time enforcement where the general case cannot, for two reasons. It is
...
The lint SHALL stay narrow deliberately. It flags only a callee beginning with an uppercase letter,
...
Enforced by: `src/wgsl_lint.nim` with `tests/test_wgsl_lint.nim` (build-time, this class only);
...
- **WHEN** a source shader declares a binding the pipeline does not provide
- **THEN** the build still succeeds and the failure appears at pipeline init, exactly as before
