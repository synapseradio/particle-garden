...
- `src/ui/input/midi_core.nim`: pure. Parses raw MIDI messages, three-byte channel messages and one-byte system real-time messages, into typed events (control change, note on, note off, program change, clock, start, continue, stop), rejecting malformed input at the boundary. Natively tested.
...
type
  ...
  ControlRow* = object
    ...
    of rkModulate:
      ...
      attackMs*: float         ## time constant while the source rises; 0 passes the raw value
      releaseMs*: float        ## time constant while the source falls; 0 passes the raw value
    of rkWrite:
      writeParamId*: string
    ...
    of rkTour:
      ...
      runningParamId*: string  ## the declared gate id whose state gates the advance; no boolean descriptor exists on the tree (ParamKind is int or float), so the gate set is declared beside the tours
...
Beat clock delivers on `midi:clock` as an event with magnitude 1.0 and the pulse's position inside the beat as ordinal. MIDI beat clock runs at 24 pulses per quarter note, sent as one-byte real-time messages, clock 0xF8 with start 0xFA, continue 0xFB, and stop 0xFC (https://en.wikipedia.org/wiki/MIDI_beat_clock, a summary of the MIDI 1.0 specification), and Web MIDI hands each to the port handler as "a Uint8Array containing the MIDI data bytes of a single MIDI message" (https://www.w3.org/TR/webmidi/), so a one-byte message reaches the core as a one-element array. The family counts pulses: start resets the count so the next clock is ordinal 0, the downbeat, continue leaves it, stop holds delivery until continue or start, and a stream that sends clock with no start counts from its first pulse, phase unknown. A Fire row selecting ordinal 0 therefore runs once per beat, and any clock-sending sequencer or drum machine supplies the beat the audio family leaves out of scope. Bars stay out, since the wire carries no time signature.

Rejected: a continuous `midi:beat` phase source. A phase needs a tempo estimate to interpolate between pulses, and the ordinal already gives a Fire row the pulse grid at 24 per beat.

MIDI declares in two moments. The shipped mapping's source ids and `midi:clock` are compile-time constants declared at wiring time, which is the declaration set D8's static gate validates the shipped rows against, and which keeps a shipped row resolved before any hardware has spoken. Every other control declares the first time it sends, re-registering the family with the grown set under the contract's replace-on-re-registration rule. The editor's source list therefore carries the shipped ids plus the vocabulary the connected hardware has actually used, and a stored row naming neither stays unresolved and visible.
...
The shipped default matrix is a `const` in `src/ui/input/shipped_mapping.nim` (corrected at tasking, 2026-09-12: it names MIDI ids, which the family-blind matrix module must not, and its static gate needs the descriptor table, the MIDI declarations and the tour declarations in one place), loaded when storage is empty or invalid. Its contents: four Write rows with soft takeover put the coupling strengths on channel 1's hardware-common knobs, `midi:cc:1:7` (volume) to `forceStrength`, `midi:cc:1:1` (mod wheel) to `fluidStrength`, `midi:cc:1:74` (cutoff) to `rdFieldForce`, and `midi:cc:1:71` (resonance) to `rdDeposit`, since stock controllers emit these without configuration and the garden's silence leaves their audio meanings colliding with nothing. Volume lands on the same target as `audio:loudness` and cutoff on the same target as `audio:brightness`, so level means the dance's energy and brightness means field-heed whether they arrive from a knob or from the room, a pairing that reads unexplained because both sides measure the same kind of thing (Fels et al. on transparency, via https://www.seeingsound.co.uk/docs/Audiovisual_Particles.pdf, section 2.3). Six Fire rows put the regimes on `midi:pc:1`, ordinals 0 through 5. One Touch row lays a 4 x 4 grid on `midi:notes:1` from `baseNote` 36, the range common pad controllers start at. Toggles ship unmapped and arrive through learn, so a stray note never flips the picture. A static gate asserts every shipped row survives validation against the live descriptor table and the compile-time source declarations D3 gives the shipped ids, in the style of the descriptor table's own compile-time gates.
...
The one stored mapping travels as a document. The boundary serves `exportMappingJson()` and `applyMappingJson(text)` beside the preset pair it mirrors (`exportPresetJson`, `applyPresetJson`, `src/web_api.nim:1323-1327`), the apply arm running the same validate-first decode as a load from storage and answering the same `{ok, error}` shape. The panel offers the text for download and accepts a file, then persists what the boundary answered through the ordinary edit path. MilkDrop's author credits the community authoring and trading presets for the visualizer's survival (https://www.geisswerks.com/about_milkdrop.html), and a mapping handed between two players is the same trade for this instrument. A named mapping library stays a non-goal; a document is enough to hand over.
...
Arming captures the set of continuous sources that delivered in the frame before the arm, and those sources never qualify for that arming. The audio family delivers five continuous values every frame while listening (openspec/changes/audio-interface/specs/audio-input/spec.md, the registration requirement), so without the capture an arm made with the microphone on binds `audio:loudness` before the user touches anything, and a controller streaming aftertouch or a stuck knob does the same. Audio sources are picked from the editor's source list instead. The capture is matrix-side and needs nothing from any family, so the contract line that a family needs no learn awareness holds.

Rejected: a learnable flag on the family declaration. It changes the registration contract both changes build on, and it misses the stuck-knob case that the capture covers for free.
...
- `tests/test_midi_core.nim`: three-byte and one-byte parsing, malformed input rejected at the boundary, normalization to [0, 1], channel extraction into source ids, and the clock count across start, continue, and stop.
- `tests/test_control_matrix.nim`: the five validation relations of D2, arbitration summing and clamping in travel space against real descriptors, rank ordering across Write and Tour rows on one parameter, takeover engagement and disengagement across crossing, preset apply, and tour movement, attack and release convergence on a rising and a falling source, Tour phase independence and the gate's effect on advance, schema round-trip, the drop-and-clamp decode stance, and the shipped defaults surviving validation.
...
**Flush.** `web_api.flushMatrix(dtSeconds)` runs once per frame from the frame loop with the frame's capped wall-clock delta (the tour advance and the row envelopes consume it), after every family's delivery for the frame and before physics, and is the frame's only parameter writer. Families never call it and never observe it. Everything downstream of delivery, arbitration, takeover, excursions, writes, gestures, shading, and the widened push, is matrix-side.
...
3. **A Fire row that waits for the next onset.** Every Fire action lands on the frame its event arrives. Geiss built its next warp map in the background and then "would wait for a good beat, and then it would suddenly switch" (https://www.geisswerks.com/geiss/secrets.html), so a structural change landed on the music. A Fire row option that holds a fired action until the next event on a named event source (a regime change or a reseed landing on `midi:clock` ordinal 0, or on `audio:onset`) would carry that. `midi:clock` is the exact grid such an option wants. Whether it is a row field or a second row kind stays open.
