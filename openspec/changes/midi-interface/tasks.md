...
Eight places the design's assumptions and the tree disagree, with what these tasks do:
...
- [ ] 1.4 `just happen` builds and `just check` is green

## 2. The row model, pure and natively tested

- [ ] 2.1 **Red first.** Write `tests/test_control_matrix.nim` over a module that does not exist yet:
...
- [ ] 2.2 Write `src/ui/input/control_matrix.nim`, pure on both backends in the style of
...
- [ ] 2.3 **Red first.** Extend `tests/test_control_matrix.nim` with delivery: a sweep collapsing to
...
- [ ] 2.4 Add the `test_control_matrix.nim` row to the file table and the architecture tree in
      `tests/README.md`. Verify by reading the table
- [ ] 2.5 `just happen` builds and `just check` is green

## 3. Arbitration, envelopes, takeover and the pure flush

- [ ] 3.1 **Red first.** Extend `tests/test_control_matrix.nim` over a `flushMatrix(state, ctx)` that
...
- [ ] 3.2 Add the modulate and write arms to `src/ui/input/control_matrix.nim`: base travel from
...
- [ ] 3.3 **Red first.** Extend the suite with takeover and rank: a resting soft-takeover row moving
      nothing, engagement by crossing and by landing within one `positionStep`, disengagement across a
...
- [ ] 3.4 **Red first.** Extend the suite with tours, fires and touches: a running `Tour` row
      advancing its own phase on the context's wall-clock delta scaled by its speed and writing every
      axis of `pointAt(phase)` in one batch; a false gate freezing the phase and writing nothing; two
      ...
      run green
- [ ] 3.5 `just happen` builds and `just check` is green

## 4. The mapping document, its schema and the shipped default

- [ ] 4.1 **Red first.** Extend `tests/test_control_matrix.nim` with the schema relations, in
...
- [ ] 4.2 Add the versioned schema to `src/ui/input/control_matrix.nim` following
...
- [ ] 4.3 Write `src/ui/input/shipped_mapping.nim`, pure, importing `control_matrix`,
      ...
      `midi:clock` with their kinds and labels; `DEFAULT_MAPPING` holding the four soft-takeover
      `Write` rows onto `forceStrength`, `fluidStrength`, `rdFieldForce` and `rdDeposit`, six `Fire`
      rows on `midi:pc:1` ordinals 0 through 5 selecting the `RD_REGIMES` ids in table order, and one
      `Touch` row laying a 4 by 4 grid on `midi:notes:1` from base note 36, with tour rows ranked
      below write rows and no toggle mapped; and the tour declarations for the two weathers built from
...
- [ ] 4.4 Add the static gate to `src/ui/input/shipped_mapping.nim`: a `static:` block asserting every
      ...
      **Watch it fail:** point one shipped row at a parameter id nothing serves, confirm `just test`
      ...
      task notes
- [ ] 4.5 **Red first.** Extend `tests/test_control_matrix.nim`: the default mapping validates, an
...
- [ ] 5.1 In `src/web_api.nim` hold the live `MatrixState` and add the contract's entry points:
      ...
      float; ordinal: int)` queueing in arrival order. Register the two tour declarations from
      `shipped_mapping` at module scope, load `DEFAULT_MAPPING` there, and resolve the targeted
      descriptors once at module scope the way `climateDescriptors` already is
      (`src/web_api.nim:712-716`). Verify `just build-app`
- [ ] 5.2 In `src/web_api.nim` add `flushMatrix(dtSeconds: float)`: build the context from
...
- [ ] 5.3 In `src/web_api.nim` add the action and gesture arms of the flush: a `case` over
...
- [ ] 5.4 In `src/web_api.nim` serve the mapping surface on `gardenAPI`: the active mapping's rows
      ...
      `just build-ui`
- [ ] 5.5 In `src/ui/input/control_matrix.nim` add learn: arming captures the set of continuous
...
- [ ] 5.6 In `src/web_api.nim` add the `excursions` record to `pushStats` beside `ceilings`: the
...
- [ ] 5.7 In `src/app.nim` call `web_api.flushMatrix(cappedDt)` directly after
...
- [ ] 6.1 **Red first.** Add a suite to `tests/test_control_matrix.nim` that reads `src/app.nim` from
...
- [ ] 6.2 In `src/app.nim` delete the two `if config.CONFIG.climateDrift` / `forceWeather` branches
...
- [ ] 6.3 In `src/web_api.nim` delete `setClimateFromSimulation`, `setForceWeatherFromSimulation` and
      ...
      name (`grep -rn`)
- [ ] 6.4 In `src/web_api.nim` replace the two enumerated loops in `pushStats`
      ...
      and `just build-app`
- [ ] 6.5 In `docs/enforcement.md` change the "Weather tours and their step ceilings" home row to name
...
- [ ] 6.6 `just happen` builds and `just check` is green
...
- [ ] 7.1 **Red first.** Write `tests/test_midi_core.nim`: a control-change triple parsing to channel,
...
- [ ] 7.2 Write `src/ui/input/midi_core.nim`, pure on both backends and importing no transport:
...
- [ ] 7.3 Write `src/bindings/web_midi.nim` in the style of `src/bindings/web_audio.nim`:
...
- [ ] 7.4 Write `src/midi_input.nim` (layer 3, imported in `src/app.nim` after `audio_input`) beside
      ...
      and registration of `shipped_mapping.SHIPPED_MIDI_SOURCES` at module init with re-registration
      of the grown set the first time an undeclared control sends. Register the connect hooks with
      `web_api` at module init the way `registerAudioControl` does, and expose connect, disconnect and
      the state read on `gardenAPI`, typed in `web-ui/src/garden-api.ts`. Verify `just build-app` and
      `just build-ui`
- [ ] 7.5 In `docs/enforcement.md` add `src/ui/input/control_matrix.nim` and
      ...
      tables
- [ ] 7.6 `just happen` builds and `just check` is green

## 8. The panel's mapping editor, shading and help

- [ ] 8.1 **Red first.** Write `web-ui/test/mapping-editor.test.ts` over a pure helper
...
- [ ] 8.2 **Red first.** Add an excursion case to `web-ui/test/bounds.test.ts` over a helper in
...
- [ ] 8.3 In `web-ui/src/components/ParamSlider.tsx` shade the live excursion from
...
- [ ] 8.4 Write `web-ui/src/components/MidiSection.tsx`: a connect control as a `toggle-label`
      ...
      `nim c -r tests/test_panel_reachability.nim` green
- [ ] 8.5 **Red first.** Add the D12 relation to `tests/test_help_content.nim`: the MIDI help file
      exists, and its body contains every `DEFAULT_MAPPING` row's target — descriptor ids for the four
      `Write` rows, action ids for the six `Fire` rows, the source id for the `Touch` row. Add `"midi"`
...
- [ ] 8.6 Write `docs/help/70-midi.md` with the front matter `group: midi`: what connecting does and
...
- [ ] 8.7 `just happen` builds and `just check` is green
...
- [ ] 9.1 `just happen`, launch `./main` in the background, poll the port, navigate the connected tab
      ...
      disconnected, the eleven shipped rows listed with their targets, every one resolved before any
