...
- [ ] 2.3 **The switch, in one commit** (design N3 step 2):
  ...
  - `src/body_core.nim:195-199` loses `BODY_LARGEST_FRAME_FACTOR`, with `BODY_MAX_FORCE_PER_PARTICLE = 2 · BODY_FORCE_CEILING · BODY_STRENGTH_CEILING`.
...
- [ ] 2.4 **Red, then the coarse word** (design C8, N3 step 3):
...
- [ ] 6.1 **Red.** Add three suites to `tests/test_long_range_core.nim`, each with its tolerance and measured source beside it:
  - "The Pull Does Not Depend On Mesh Size": every size in `LR_GRID_SIZES`, sampled 240 and 600 from the clump's centre, within 0.15% and 0.10%
...
- [ ] 7.5 Set the gains, ranges and defaults (N2, N7):
  ...
  - **Word budget.** The scent and long-range word budgets from 2.4 are re-budgeted at `F_c`.
