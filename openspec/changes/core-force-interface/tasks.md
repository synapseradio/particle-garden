...
- [ ] 4.4 **Gate G1.1, the onset; it gates 4.7.** Add one reporting arm under `calibrateBalance` that asserts nothing. On the gate seeds at 128 000 particles and radius 50, polynomial model, one and four species, no coupling but the species force, it prints the p99.9 of `x` at the end of the window, the floor at the preset `repulsionEnd`, and the band's lower edge. Record them in `scratchpad/core-force-interface/g1-onset__<DD-MM-YY-HHmm>.md`, stating that other counts, radii, species counts and the exponential model are unmeasured. `x_on = 6.3` is the user's placement and is not derived from this run: the record states which settles it lies between (below every one-species settle, above every four-species diagonal-only settle).
