...
| Every coupling strength's range reaches zero, so each coupling turns off through its own slider | Build-asserted | A static loop over the six floors at the bottom of `src/config_ranges.nim`; crowding, which shapes the force law instead of gating a pass, is asserted beside it | |
...
| A full crowd cannot overflow a body's force or torque accumulator | Build-asserted | Static assertions in `src/body_core.nim` relating `MAX_PARTICLES`, the largest contribution the ranges admit (`BODY_MAX_FORCE_PER_PARTICLE`, one `BODY_FORCE_CEILING` per particle per body, since proximity's and enclosure's eases sum to at most one), the world's half-diagonal and the two fixed-point scales | |
| A body's pull on a particle is bounded in size and in region: at most one `BODY_FORCE_CEILING` times strength and envelope, and exactly zero past twice the band in the evaluation's distance | Test-held on the mirror for one body; Unenforced for the sum across slots | In `tests/test_body_core.nim`, the reach, peak, corner and ceiling-sweep tests in "One Evaluation Yields Both Proximity And Enclosure", and "overlapping bodies add and a body out of reach adds nothing" in "A Body Is Blind Past Its Reach". The sum across slots is `body-force.wgsl`'s loop, which the mirror has no entry for, so the pair holds it by review (Reference oracles) | A multi-body entry in `body_core` whose slot loop the shader's mirrors, held by the overlap test |
...
| A full crowd at the range maxima fits both velocity words, and decodes to its impulse | Build-asserted, and Test-held on the mirror | The word assertions at the bottom of `src/config_ranges.nim` over each writer's named maximum; "A Full Crowd Decodes To Its Impulse" in `tests/test_physics.nim` sums a full crowd through the split and decode oracles with wrapping adds. The shaders' split and decode follow the oracles by review (Reference oracles) | |
