...
- `fluidStrength` — how much of the fluid's push reaches the particles.
  ...
  Interacts with: the other fluid sliders (dims them at zero); Long Range
  (fluid loosens its clusters and makes it cheaper); MIDI and audio (move
  it).
...
- `sphStiffness` — how hard the fluid resists compression. Its usable
  ceiling depends on the Interaction Radius, the fluid's reach and the time
  scale, so the slider shades the range the current settings cannot hold.
  Interacts with: Fluid Scale, Interaction Radius and Time Scale (all move
  its ceiling); Rest Density.
