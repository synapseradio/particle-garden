...
- **When subagents carry the work, tests run once, at the end — not once per subagent.** The integrating session runs `just happen` and `just check` a single time over the merged result. A subagent compiles and runs at most the test module it is changing; it does not run the full build and both suites.
