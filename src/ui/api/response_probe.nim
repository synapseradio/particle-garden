...
# and must-fail anchor sets; the calibration section legibilityReportMarkdown
# emits into docs/control-legibility-report.md records where each sits.
# ------------------------------------------------------------------------------
...
const
  ...
  SPAN_MIN* = 0.15
    ## The track's ends must differ by at least this fraction of the
    ## reference magnitude.
  LIVE_FRACTION_MIN* = 0.70
    ## At least this fraction of the travel must move the observable.
  CLIFF_MAX* = 0.25
    ## No single movement may jump the observable by more than this fraction
...
func defaultProbeContext*(): ProbeContext =
  ...
  ## No strength lift is needed: every strength multiplying a probed
...
proc legibilityReportMarkdown*(
  ...
  result.add "Span, live fraction, and cliff per declared slice, " &
    ...
    " (the calibration section below records where each sits). " &
    "Regenerated by tests/test_response_probe.nim; edits here " &
    "are overwritten.\n\n"
  ...
  result.add "Each threshold sits inside the measured gap between the " &
    "must-pass and must-fail anchor sets.\n\n"
  result.add "SPAN_MIN=" & $SPAN_MIN & ": must-fail edge rdFeed, span " &
    "0.131 on its pre-joint default slice; must-pass edge sphViscosity, " &
    "0.667 (the smallest of the must-pass set). Placed near the fail edge " &
    "so the modest-but-live resolved-deposit response (0.211) stays " &
    "outside the remedy net; every must-pass control clears the bar by at " &
    "least 4x.\n\n"
  result.add "LIVE_FRACTION_MIN=" & $LIVE_FRACTION_MIN & ": must-fail " &
    "edge rdKill, 0.514; must-pass edge 1.000 (the whole must-pass set). " &
    "0.70 sits between, above the fail edge by a third, and leaves " &
    "expAttractionBeta's measured 0.875 — a live control with a short " &
    "flat tail — clear of the bar.\n\n"
  result.add "CLIFF_MAX=" & $CLIFF_MAX & ": kept at the provisional " &
    "value, which the measurement showed already inside the gap: " &
    "must-pass cliffs reach 0.020; the must-fail edge (rdFeed) measured " &
    "1.364.\n\n"
  result.add "RESPONSE_EPSILON=" & $RESPONSE_EPSILON & ": unmoved by " &
    "calibration — neither anchor set separates on it, so the provisional " &
    "value stands until a measurement gives it an edge to sit against.\n"
